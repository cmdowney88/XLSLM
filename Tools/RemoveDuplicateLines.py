"""
Script to remove any duplicate lines between AmericasNLP files and KannEtAl2018 files for
the same language. If a line exists in both the AmericasNLP data and KannEtAl2018 data, it is 
removed from KannEtAl2018 and stays in AmericasNLP. The script is only meant for one language
at a time.
Expected command line args: <program_name> <AmericasNLP_train_file> <AmericasNLP_dev_file>
<KannEtAl2018_file> 
"""
import sys
...
def remove(
    ...
    """
    Removes duplicate lines between AmericasNLP data and Kann et al. 2018 data from the Kann
    et al. 2018 data.
    Returns the remaining lines for the Kann et al. 2018 data.
    Args:
        anlp_lines: A list of the lines in the AmericasNLP files
        kea_lines: A list of the lines in the Kann et al. file.
    Returns:
        A list of strings, which are the remaining lines from the Kann et al. file after
        removing duplicates
    """
    #to store anlp lines with no whitespace or punctuation
    stripped_anlp_lines = []

    #remove whitespace and punctuation for anlp lines for comparison with kae lines
    for line in anlp_lines:
        stripped_line = no_whitespace_or_punct(line)
        stripped_anlp_lines.append(stripped_line)

    #to store the final lines from Kann et al. 2018 remaining after removing any duplicates
    final_kea_lines = []

    #remove whitespace and punctuation for Kann et al. lines and compare with the lines
    #from AmericasNLP with whitespace and punctuation removed. If not a duplicate line, add
    #Kann et al. line to list of final lines.
...
def no_whitespace_or_punct(line: str) -> str:
    ...
    Returns a string with whitespace and punctuation removed.
    Args:
...
def main():
    """
    Main function of program. Gets files from command lines. Removes duplicate lines between
    AmericasNLP and Kann et al. 2018 from Kann et al. 2018 with remove() and writes the remaining
    lines to a file with write_to_file().
    """
