...
For each file, the script produces stats for the total number of lines, total number of tokens,
number of unique tokens, total number of characters, number of characters in unique tokens,
total number of unique characters, average length of all tokens, and average length of unique
tokens.
Each of the data files will have its stats output to a separate file.
There will also be a combined stats file with the stats for all of the files 
combined into one stats file, where the stats are separated by the original data file names.
"""
import sys
...
def get_stats(filename: str) -> Dict[str, str]:
    """
    Get stats for the data files from the XLSLM repository.
    For the given file, finds the total number of lines, total number of tokens, number 
    of unique tokens, total number of characters, number of characters in unique tokens,
    total number of unique characters, average length of all tokens, and average length 
    of unique tokens.
    Returns a dict of label, stat pairs that are both represented as strings.
    Args:
        filename: The path of the file to find stats for
    Returns:
        A dict of key, value pairs that are both strings. The keys are the labels for the stats.
        The values are the string representations of the stats.
    """
    ...
    stats_dict['Total number of characters in file'] = str(total_character_num_of_all_tokens)
    ...
    stats_dict['Total number of characters in unique tokens'] = str(total_character_num_of_unique_tokens)
    ...
    stats_dict['Average token length of all tokens in file'] = str(avg_token_length)
    ...
    stats_dict['Average token length of all unique tokens in file'] = str(avg_unique_token_length)
...
def write_to_file(
    ...
    Args:
        ...
        stats_dict: The dictionary of string keys and values to write to the file
    """
    f = open(filename, 'w')
...
def write_all_to_file(
    filename: str,
    list_of_stats_dicts: List[Dict[str, str]]
):
    """
    Write lines to a file from a list of dictionaires with string keys and string values.
    For each dictionary in the list, prints each key, value pair to a line.
    Args:
        filename: The name of the file to write to
        list_of_stats_dicts: The list of dictionaries with string keys and values to write
            to the file.
    """
...
def main():
    """
    Main function of the program. Gets the names of files to find stats for from the command
    line arguments. For each file, it finds the stats with get_stats() and writes them to 
    a file with write_to_file(). Also writes stats from all files to combined stats file with 
    write_all_to_file(). 
    """
